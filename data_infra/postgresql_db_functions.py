...
def get_feedback_for_user(user_id:int):
    ...
    pg_cursor.execute(f"SELECT fb.id, fb.video_id, fb.created_at, fb.updated_at, fb.overall_performance, fb.delivery_and_presentation, fb.response_quality, fb.audio_quality, fb.visual_environment, fb.post FROM feedback as fb, videos as vds WHERE (fb.video_id = vds.id AND vds.owner_id={user_id});")
